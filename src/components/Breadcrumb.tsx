    ...
    <div className="bg-[#FAF9F6] border-b border-gray-200">
      <div className="max-w-[1200px] mx-auto px-6 py-4">
        <nav className="flex items-center space-x-2 text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>
          <Link to="/" className="text-[#2B5D8C] hover:text-[#2B5D8C] hover:underline transition-colors font-medium">
            Home
          </Link>
            ...
            {items.map((item, index) => (
              <div key={index} className="flex items-center space-x-2">
                ...
                {item.path && index < items.length - 1 ? (
                  <Link to={item.path} className="text-[#2B5D8C] hover:text-[#2B5D8C] hover:underline transition-colors font-medium">
                    {item.label}
                  </Link>
                ) : (
