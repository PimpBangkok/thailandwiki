      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {articles.map((article, index) => (
            ...
            >
              <span className="text-gray-800 group-hover:text-[#2B5D8C] font-medium transition-colors">
                {article.title}
              </span>
              <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-[#2B5D8C] group-hover:translate-x-1 transition-all flex-shrink-0 ml-2" />
            </Link>
          ))}
        </div>
      </div>
